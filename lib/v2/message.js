...
var Message = function (client) {
	...
	this._createVoiceApplication = function (auth, data) {
		return this._makeIrisRequest({
			path       : "applications",
			method     : "POST",
			auth       : auth,
			body: {
				"Application": {
					"AppName": {_text: data.name},
					"ServiceType": {_text: 'Voice-V2'},
					"CallInitiatedCallbackUrl": {_text: data.callbackUrl},
					"CallInitiatedMethod": {_text: 'POST'},
					"CallBackCreds": data.callbackAuthData ? {
						"UserId": {_text: data.callbackAuthData.userName},
						"Password": {_text: data.callbackAuthData.password}
					} : null
	...
	this._setVoiceOriginationSetting = function (auth, application) {
		return  this._makeIrisRequest({
			path       : "sites/" + auth.subaccountId + "/sippeers/" + application.locationId + "/products/origination/settings",
			method     : "POST",
			auth       : auth,
			body: {
				"SipPeerOriginationSettings": {
					"VoiceProtocol": 'HTTP',
					"HttpSettings": {
						"HttpVoiceV2AppId": {_text: application.applicationId},
					}
				}
			}
		});
	...
	*/
	this.createMessagingApplication = function (auth, data, callback) {
		...
		var self = this;
		return Promise.all([
			self._createApplication(auth, data),
			self._createLocation(auth, data)
		])
			.then(function (result) {
				app = {
	...
	/**
	* Enable MMS for an application
	* @param {Object} auth Bandwidth Dashboard auth data {accountId: "accountId",	userName: "userName", password: "password",	subaccountId: "subaccountId"}
	* @param {Object} data Options of new application
	* @param {String} data.locationId Location Id
	* @returns {Object} Enable MMS response
	*
	* @example
	* client.v2.Message.enableMms(authData, {
	*   locationId: "My Voice App"
	* }).then(function (response) {});
	*/
	this.enableMms = function (auth, application, callback) {
		return this
			._enableMms(auth, { enabled: true }, application)
			.asCallback(callback);
	};

	/**
	* Create voice application on Bandwidth Dashboard (its id is required to send messages)
	* @param {Object} auth Bandwidth Dashboard auth data {accountId: "accountId",	userName: "userName", password: "password",	subaccountId: "subaccountId"}
	* @param {Object} data Options of new application
	* @param {String} data.name Application name
	* @param {String} data.callbackUrl callback url to receive messages events
	...
	*
	* @example
	* client.v2.Message.createVoiceApplication(authData, {
	*   name: "My Voice App",
	*   callbackUrl: "http://your/callback/handler"
	* }).then(function (application) {});
	*/
	this.createVoiceApplication = function (auth, data, callback) {
		var voiceApp;
		var self = this;
		return self._createVoiceApplication(auth, data)
			.then(function (result) {
				voiceApp = {
					applicationId: result,
					locationId: data.locationId
				};
				return self._setVoiceOriginationSetting(auth, voiceApp);
			})
			.then(function () {
				return voiceApp;
			})
